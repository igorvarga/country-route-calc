...
public class RemoteCountrySource implements CountrySource {
  ...
  public RemoteCountrySource(
      CountriesConfig config, RestClient.Builder builder, ObjectMapper objectMapper) {
    this.config = config;
  ...
  public CountryDataset load() {
    if (config.remote() == null || config.remote().url() == null) {
      throw new IllegalStateException(
          "countries.remote.url is required when countries.source=remote");
    }
